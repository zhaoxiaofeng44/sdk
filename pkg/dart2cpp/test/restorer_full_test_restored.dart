...
void Printable_printInfo(dynamic this_) {
  print(
      '[${(this_.vptr['get_displayName'] as String Function(dynamic))(this_)}]');
}

// mixin Orderable → static functions for delegation
bool Orderable_isLessThan<T>(dynamic this_, T other) {
  return ((this_.vptr['compareTo'] as int Function(dynamic, T))(this_, other) <
      0);
}
...
class Vector2DValue {
  ...
  bool operator ==(Object other) {
    final eqFn = vptr['operatorEq'];
    if (eqFn != null) return (eqFn as Function)(this, other) as bool;
...
Vector2DValue Vector2D_operatorPlus(Vector2DValue this_, Vector2DValue other) {
  return (() {
    final _obj = Vector2DValue();
    Vector2D_new(_obj, (this_.x + other.x), (this_.y + other.y));
    return _obj;
  })();
}
...
Vector2DValue Vector2D_operatorStar(Vector2DValue this_, double scalar) {
  return (() {
    final _obj = Vector2DValue();
    Vector2D_new(_obj, (this_.x * scalar), (this_.y * scalar));
    return _obj;
  })();
}

bool Vector2D_operatorEq(Vector2DValue this_, Object other) {
  return (((other is Vector2DValue) && (this_.x == other.x)) &&
      (this_.y == other.y));
}

double Vector2D_get_length(Vector2DValue this_) {
  return ((((this_.x * this_.x) + (this_.y * this_.y)) < 0)
      ? 0.0
      : Vector2D__sqrt(((this_.x * this_.x) + (this_.y * this_.y))));
}
...
CounterValue Counter_new(String label, {int initialValue = 0}) {
  return (() {
    final _obj = CounterValue();
    Counter_new__(_obj, label, initialValue);
    return _obj;
  })();
}
...
CounterValue Counter_new_fromString(String spec) {
  ...
  return (() {
    final _obj = CounterValue();
    Counter_new__(_obj, parts[0], int.parse(parts[1]));
    return _obj;
  })();
}
...
void Result_new_success<T>(ResultValue<T> this_, T value) {
  this_.vptr = {
    'fold': (self,
            {required dynamic Function(T) onSuccess,
            required dynamic Function(String) onFailure}) =>
        Result_fold(self, onSuccess: onSuccess, onFailure: onFailure),
...
R Result_fold<T, R>(ResultValue<T> this_,
    {required R Function(T) onSuccess, required R Function(String) onFailure}) {
...
String Result_toString<T>(ResultValue<T> this_) {
  return (this_.isSuccess
      ? 'Result.success(${this_.data})'
      : 'Result.failure(${this_.error})');
}
...
void BoundedValue_new(
    BoundedValueValue this_, double min, double max, double initial) {
  this_.vptr = {
...
void BoundedValue_set(BoundedValueValue this_, double value) {
  assert(((value >= this_.min) && (value <= this_.max)),
      'value ${value} out of bounds [${this_.min}, ${this_.max}]');
...
void Polygon_new(PolygonValue this_, String color, int sides,
    {double opacity = 1.0}) {
...
void RegularPolygon_new(
    RegularPolygonValue this_, String color, int sides, double sideLength,
    {double opacity = 1.0}) {
...
double RegularPolygon_perimeter(RegularPolygonValue this_,
    [double? overrideSideLength = null]) {
...
void Square_new(SquareValue this_, String color, double size,
    {double opacity = 1.0}) {
...
DataPointValue DataPoint_clone(DataPointValue this_) {
  return (() {
    final _obj = DataPointValue();
    DataPoint_new(_obj, this_.x, this_.y, this_.label);
    return _obj;
  })();
}
...
void Loggable_log(dynamic this_, String message) {
  print(
      '[${(this_.vptr['get_logTag'] as String Function(dynamic))(this_)}] ${message}');
}

// mixin Validatable → static functions for delegation
bool Validatable_validate(dynamic this_) {
  return (this_.vptr['serialize'] as String Function(dynamic))(this_)
      .isNotEmpty;
}

class LoggedDataPointValue
    extends LoggedDataPoint_DataPoint_Loggable_ValidatableValue {
  late Map<String, dynamic> vptr;
  late double x;
  late double y;
  late String label;
...
void LoggedDataPoint_new(
    LoggedDataPointValue this_, double x, double y, String label) {
...
int LoggedDataPoint_compareTo2(
    LoggedDataPointValue this_, DataPointValue other) {
...
void Config_new(ConfigValue this_, String host, int port,
    {bool secure = false}) {
  this_.vptr = {
...
void SortedList_add<T extends Comparable<dynamic>>(
    SortedListValue<T> this_, T item) {
  this_._items.add(item);
  this_._items.sort();
}

T SortedList_get_first<T extends Comparable<dynamic>>(
    SortedListValue<T> this_) {
...
int SortedList_get_length<T extends Comparable<dynamic>>(
    SortedListValue<T> this_) {
  return this_._items.length;
}

List<T> SortedList_toList<T extends Comparable<dynamic>>(
    SortedListValue<T> this_) {
  return List.unmodifiable(this_._items);
}

String SortedList_toString<T extends Comparable<dynamic>>(
    SortedListValue<T> this_) {
...
void NullSafetyDemo_new(NullSafetyDemoValue this_, String nonNullField,
...
void Pipeline_new<TInput, TOutput>(
    PipelineValue<TInput, TOutput> this_, TOutput Function(TInput) _transform) {
  this_.vptr = {
...
TOutput Pipeline_execute<TInput, TOutput>(
    PipelineValue<TInput, TOutput> this_, TInput input) {
  return (() {
    final _let4 = input;
    return this_._transform(_let4);
  })();
}

PipelineValue<TInput, TNewOutput> Pipeline_then<TInput, TOutput, TNewOutput>(
    PipelineValue<TInput, TOutput> this_, TNewOutput Function(TOutput) next) {
  return (() {
    final _obj = PipelineValue<TInput, TNewOutput>();
    Pipeline_new(_obj, ClosureEnv_anon_0(this_, next));
    return _obj;
  })();
}
...
String BitFlags_toString(BitFlagsValue this_) {
  ...
  if ((this_.vptr['has'] as bool Function(BitFlagsValue, int))(this_, 1))
    parts.add('r');
  if ((this_.vptr['has'] as bool Function(BitFlagsValue, int))(this_, 2))
    parts.add('w');
  if ((this_.vptr['has'] as bool Function(BitFlagsValue, int))(this_, 4))
    parts.add('x');
...
void ImportantEvent_new(
    ImportantEventValue this_, String name, Priority priority) {
...
class LoggedDataPoint_DataPoint_Loggable_ValidatableValue
    extends LoggedDataPoint_DataPoint_LoggableValue {
  late Map<String, dynamic> vptr;
  late double x;
  late double y;
  late String label;
...
class Event_Object_Timestamped_TaggedValue
    extends Event_Object_TimestampedValue {
  late Map<String, dynamic> vptr;
  late List<String> _tags;
}
...
String formatMessage(String template,
    [String? subject = null, int? count = null]) {
...
Map<String, int> buildScoreMap(List<String> names, bool addBonus) {
  return (() {
    final Map<String, int> _v16 = <String, int>{};
...
void main() async {
  ...
  final DogValue dog1 = (() {
    final _obj = DogValue();
    Dog_new(_obj, 'Rex', 3, 'Labrador');
    return _obj;
  })();
  final DogValue dog2 = (() {
    final _obj = DogValue();
    Dog_new(_obj, 'Max', 5, 'Poodle');
    return _obj;
  })();
  (dog1.vptr['printInfo'] as void Function(Dog_Animal_PrintableValue))(dog1);
  print(
      '${(dog1.vptr['speak'] as String Function(DogValue))(dog1)} (${dog1.breed})');
  print(
      'dog1 < dog2: ${(dog1.vptr['isLessThan'] as bool Function(Dog_Animal_Printable_OrderableValue, DogValue))(dog1, dog2)}');
  print(
      'dog1 > dog2: ${(dog1.vptr['isGreaterThan'] as bool Function(Dog_Animal_Printable_OrderableValue, DogValue))(dog1, dog2)}');
  final CatValue cat = (() {
    final _obj = CatValue();
    Cat_new(_obj, 'Whiskers', 2);
    return _obj;
  })();
  (cat.vptr['printInfo'] as void Function(Cat_Animal_PrintableValue))(cat);
  print(
      '${(cat.vptr['speak'] as String Function(CatValue))(cat)}, mood: ${(cat.vptr['get_mood'] as String Function(CatValue))(cat)}');
  (cat.vptr['set_mood'] as void Function(CatValue, String))(cat, 'sleepy');
  print(
      'mood after set: ${(cat.vptr['get_mood'] as String Function(CatValue))(cat)}');
  print('\n--- 2. operator 重载 ---');
  final Vector2DValue sum = ((() {
    final _obj = Vector2DValue();
    Vector2D_new(_obj, 3.0, 4.0);
    return _obj;
  })()
          .vptr['operatorPlus']
      as Vector2DValue Function(Vector2DValue, Vector2DValue))((() {
    final _obj = Vector2DValue();
    Vector2D_new(_obj, 3.0, 4.0);
    return _obj;
  })(), (() {
    final _obj = Vector2DValue();
    Vector2D_new(_obj, 1.0, 2.0);
    return _obj;
  })());
  final Vector2DValue diff = ((() {
    final _obj = Vector2DValue();
    Vector2D_new(_obj, 3.0, 4.0);
    return _obj;
  })()
  ...
  final Vector2DValue scaled = ((() {
    final _obj = Vector2DValue();
    Vector2D_new(_obj, 3.0, 4.0);
    return _obj;
  })()
          .vptr['operatorStar']
      as Vector2DValue Function(Vector2DValue, double))((() {
    final _obj = Vector2DValue();
    Vector2D_new(_obj, 3.0, 4.0);
    return _obj;
  })(), 2.0);
  ...
  print('v1.length = ${((() {
    ...
    Vector2D_new(_obj, 3.0, 4.0);
    return _obj;
  })().vptr['get_length'] as double Function(Vector2DValue))((() {
    final _obj = Vector2DValue();
    Vector2D_new(_obj, 3.0, 4.0);
    return _obj;
  })()).toStringAsFixed(2)}');
  ...
  print('ok: ${(() {
    final _obj = ResultValue<int>();
    Result_new_success(_obj, 42);
    return _obj;
  })()}');
  print('err: ${(() {
    final _obj = ResultValue<int>();
    Result_new_failure(_obj, 'not found');
    return _obj;
  })()}');
  final String okMsg = ((() {
    final _obj = ResultValue<int>();
    ...
    return _obj;
          ...
          .vptr['fold'] as Function)((() {
    final _obj = ResultValue<int>();
    Result_new_success(_obj, 42);
    return _obj;
  })(),
      onSuccess: (int d) => 'got ${d}', onFailure: (String e) => 'error: ${e}');
  final String errMsg = ((() {
    final _obj = ResultValue<int>();
    Result_new_failure(_obj, 'not found');
    return _obj;
  ...
  print(buildQuery(
      ...
      params: <String, String>{'q': 'dart', 'page': '1'},
      maxWait: 10,
      secure: false));
  print('\n--- 6. sync* 生成器 ---');
  ...
  final Map<String, int> scores =
      buildScoreMap(<String>['Alice', 'Bob', 'Carol'], true);
  print('scores: ${scores}');
  print('\n--- 12. late 变量 ---');
  final LazyLoaderValue loader = (() {
    final _obj = LazyLoaderValue();
    LazyLoader_new(_obj);
    return _obj;
  })();
  print(
      'before init: ${(loader.vptr['get_data'] as String Function(LazyLoaderValue))(loader)}, ${(loader.vptr['get_computedValue'] as int Function(LazyLoaderValue))(loader)}');
  (loader.vptr['initialize'] as void Function(LazyLoaderValue, String))(
      loader, 'hello');
  print(
      'after init: ${(loader.vptr['get_data'] as String Function(LazyLoaderValue))(loader)}, ${(loader.vptr['get_computedValue'] as int Function(LazyLoaderValue))(loader)}');
  print('\n--- 13. rethrow ---');
  try {
    parseAndDivide('10', '2');
  ...
  print('\n--- 14. assert ---');
  final BoundedValueValue bv = (() {
    final _obj = BoundedValueValue();
    BoundedValue_new(_obj, 0.0, 10.0, 5.0);
    return _obj;
  })();
  (bv.vptr['set'] as void Function(BoundedValueValue, double))(bv, 7.5);
  print(
      'BoundedValue: ${(bv.vptr['get_current'] as double Function(BoundedValueValue))(bv)}');
  print('\n--- 15. 字符串 ---');
  print(multiLineExample());
  print('\n--- 16. typedef + 函数式组合 ---');
  final String Function(int) doubleIt =
      compose((int x) => (x * 2), (int x) => 'result=${x}');
  ...
  final List<int> nested =
      flatMap(<int>[1, 2, 3], (int x) => <int>[x, (x * x)]);
  print('flatMap: ${nested}');
  print('\n--- 19. 多层继承链 ---');
  final ShapeValue shape = (() {
    final _obj = ShapeValue();
    Shape_new(_obj, 'red');
    return _obj;
  })();
  print((shape.vptr['describe'] as String Function(ShapeValue))(shape));
  final ShapeValue transparentShape = (() {
    final _obj = ShapeValue();
    Shape_new_transparent(_obj, 'blue');
    return _obj;
  })();
  print((transparentShape.vptr['describe'] as String Function(
      ShapeValue))(transparentShape));
  final PolygonValue polygon = (() {
    final _obj = PolygonValue();
    Polygon_new(_obj, 'green', 6, opacity: 0.8);
    return _obj;
  })();
  print((polygon.vptr['describe'] as String Function(PolygonValue))(polygon));
  print(
      'perimeter: ${(polygon.vptr['perimeter'] as double Function(PolygonValue, double))(polygon, 3.0)}');
  final RegularPolygonValue hexagon = (() {
    final _obj = RegularPolygonValue();
    RegularPolygon_new(_obj, 'yellow', 6, 5.0);
    return _obj;
  })();
  print((hexagon.vptr['describe'] as String Function(
      RegularPolygonValue))(hexagon));
  print(
      'perimeter: ${(hexagon.vptr['perimeter'] as double Function(RegularPolygonValue))(hexagon)}');
  print(
      'area: ${(hexagon.vptr['area'] as double Function(RegularPolygonValue))(hexagon)}');
  final SquareValue square = (() {
    final _obj = SquareValue();
    Square_new(_obj, 'white', 10.0, opacity: 0.9);
    return _obj;
  })();
  print((square.vptr['describe'] as String Function(SquareValue))(square));
  print(
      'square perimeter: ${(square.vptr['perimeter'] as double Function(RegularPolygonValue))(square)}');
  print('\n--- 20. implements 多接口 ---');
  final DataPointValue dp1 = (() {
    final _obj = DataPointValue();
    DataPoint_new(_obj, 1.0, 2.0, 'A');
    return _obj;
  })();
  final DataPointValue dp2 = (() {
    final _obj = DataPointValue();
    DataPoint_new(_obj, 3.0, 1.0, 'B');
    return _obj;
  })();
  print('dp1: ${dp1}');
  print(
      'dp1.serialize: ${(dp1.vptr['serialize'] as String Function(DataPointValue))(dp1)}');
  final DataPointValue dp1Clone =
      (dp1.vptr['clone'] as DataPointValue Function(DataPointValue))(dp1);
  print('dp1.clone: ${dp1Clone}');
  print(
      'dp1.compareTo2(dp2): ${(dp1.vptr['compareTo2'] as int Function(DataPointValue, DataPointValue))(dp1, dp2)}');
  print('\n--- 21. mixin on 约束 ---');
  final LoggedDataPointValue ldp = (() {
    final _obj = LoggedDataPointValue();
    LoggedDataPoint_new(_obj, 5.0, 6.0, 'logged');
    return _obj;
  })();
  (ldp.vptr['log'] as void Function(
      LoggedDataPoint_DataPoint_LoggableValue, String))(ldp, 'created');
  print(
      'validate: ${(ldp.vptr['validate'] as bool Function(LoggedDataPoint_DataPoint_Loggable_ValidatableValue))(ldp)}');
  print(
      'serialize: ${(ldp.vptr['serialize'] as String Function(DataPointValue))(ldp)}');
  print('\n--- 22. 增强枚举 ---');
  print('Priority.high: ${Priority}.high');
  print(
      'high > medium: ${Priority_isHigherThan(Priority.high, Priority.medium)}');
  ...
  print('\n--- 23. 重定向构造函数 ---');
  final ConfigValue cfg1 = (() {
    final _obj = ConfigValue();
    Config_new(_obj, 'example.com', 8080);
    return _obj;
  })();
  final ConfigValue cfg2 = (() {
    final _obj = ConfigValue();
    Config_new_localhost(_obj);
    return _obj;
  })();
  final ConfigValue cfg3 = (() {
    final _obj = ConfigValue();
    Config_new_production(_obj, 'api.example.com');
    return _obj;
  })();
  print('cfg1: ${cfg1}');
  print('cfg2: ${cfg2}');
  print('cfg3: ${cfg3}');
  print('\n--- 24. 泛型约束 ---');
  final SortedListValue<int> sortedList = (() {
    final _obj = SortedListValue<int>();
    SortedList_new(_obj);
    return _obj;
  })();
  (sortedList.vptr['add'] as void Function(SortedListValue, int))(
      sortedList, 5);
  (sortedList.vptr['add'] as void Function(SortedListValue, int))(
      sortedList, 1);
  (sortedList.vptr['add'] as void Function(SortedListValue, int))(
  ...
  print(
      'first: ${(sortedList.vptr['get_first'] as dynamic Function(SortedListValue))(sortedList)}, last: ${(sortedList.vptr['get_last'] as dynamic Function(SortedListValue))(sortedList)}');
  final int maxVal = findMax(<int>[3, 7, 1, 9, 4]);
  print('findMax: ${maxVal}');
  final String result =
      applyTwice(5, (int x) => 'n=${x}', (String s) => '${s}!');
  print('applyTwice: ${result}');
  print('\n--- 25. null safety ---');
  final NullSafetyDemoValue ns1 = (() {
    final _obj = NullSafetyDemoValue();
    NullSafetyDemo_new(_obj, 'hello', 'world');
    return _obj;
  })();
  print(
      'ns1: ${(ns1.vptr['demonstrate'] as String Function(NullSafetyDemoValue))(ns1)}');
  final NullSafetyDemoValue ns2 = (() {
    final _obj = NullSafetyDemoValue();
    NullSafetyDemo_new(_obj, 'hello');
    return _obj;
  })();
  print(
      'ns2: ${(ns2.vptr['demonstrate'] as String Function(NullSafetyDemoValue))(ns2)}');
  final String? found = findFirst(
      <String>['apple', 'banana', 'cherry'], (String s) => s.startsWith('b'));
  print('findFirst(b): ${found}');
  final String? notFound =
      findFirst(<String>['apple', 'banana'], (String s) => s.startsWith('z'));
  print('findFirst(z): ${notFound}');
  print('\n--- 26. for-in + do-while ---');
  final List<int> filtered =
      filterWithForIn(<int>[5, (-3), 10, 200, 50, (-1), 80]);
  ...
  print('\n--- 29. covariant ---');
  final CircleRendererValue renderer = (() {
    final _obj = CircleRendererValue();
    CircleRenderer_new(_obj);
    return _obj;
  })();
  print(
      'renderer: ${(renderer.vptr['get_name'] as String Function(CircleRendererValue))(renderer)}');
  (renderer.vptr['render'] as void Function(CircleRendererValue, String))(
      renderer, 'circle');
  print('\n--- 30. Pipeline 泛型链 ---');
  final PipelineValue<int, String> pipeline = Pipeline_then<int, int, String>(
      Pipeline_then<int, String, int>((() {
        final _obj = PipelineValue<int, String>();
        Pipeline_new(_obj, (int n) => 'val=${n}');
        return _obj;
      })(), (String s) => s.length),
      (int len) => 'len=${len}');
  print(
      'pipeline(42): ${(pipeline.vptr['execute'] as String Function(PipelineValue, int))(pipeline, 42)}');
  print(
      'pipeline(12345): ${(pipeline.vptr['execute'] as String Function(PipelineValue, int))(pipeline, 12345)}');
  print('\n--- 31. switch-case ---');
  ...
  print('\n--- 32. 位运算 ---');
  final BitFlagsValue flags = (() {
    final _obj = BitFlagsValue();
    BitFlags_new(_obj);
    return _obj;
  })();
  ...
  print('flags: ${flags}');
  print(
      'has read: ${(flags.vptr['has'] as bool Function(BitFlagsValue, int))(flags, 1)}');
  print(
      'has write: ${(flags.vptr['has'] as bool Function(BitFlagsValue, int))(flags, 2)}');
  ...
  final EventValue event = (() {
    final _obj = EventValue();
    Event_new(_obj, 'meeting');
    return _obj;
  })();
  (event.vptr['addTag'] as void Function(
      Event_Object_Timestamped_TaggedValue, String))(event, 'work');
  (event.vptr['addTag'] as void Function(
      Event_Object_Timestamped_TaggedValue, String))(event, 'important');
  print(event);
  final ImportantEventValue impEvent = (() {
    final _obj = ImportantEventValue();
    ImportantEvent_new(_obj, 'deadline', Priority.critical);
    return _obj;
  })();
  (impEvent.vptr['addTag'] as void Function(
      Event_Object_Timestamped_TaggedValue, String))(impEvent, 'urgent');
  (impEvent.vptr['log'] as void Function(
      ImportantEvent_Event_LoggableValue, String))(impEvent, 'created');
  print(impEvent);
  print('\n=== 所有测试通过 ✅ ===');
}
...
class ClosureEnv_anon_0<TNewOutput, TOutput, TInput> {
  ...
  TNewOutput call(TInput input) =>
      ClosureEnv_anon_0_call<TNewOutput, TOutput, TInput>(this, input);
}

TNewOutput ClosureEnv_anon_0_call<TNewOutput, TOutput, TInput>(
    ClosureEnv_anon_0<TNewOutput, TOutput, TInput> env, TInput input) {
  return env.next((() {
    final _let5 = input;
    return env.this_._transform(_let5);
  })());
}
...
C ClosureEnv_compose_1_call<C, B, A>(
    ClosureEnv_compose_1<C, B, A> env, A input) {
