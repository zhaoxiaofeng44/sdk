...
public:
  ...
  Nullable setMajor(String value) {
...
Nullable testAdditionalFunctions() {
  dart_print(dart_string("\n--- 附加函数特性 ---"));
auto greet = [&](String name) -> String return dart_concat(dart_string("你好, "), (name).toString(), dart_string("!"));;
auto addThreeNumbers = [&](Int a, Int b, Int c) -> Int return a->operator_add(b)->operator_add(c);;
...
Nullable testAdditionalGenerics() {
  dart_print(dart_string("\n--- 附加泛型特性 ---"));
auto stringHolder = ObjectPtr<GenericHolder>(new GenericHolder(dart_string("Hello")));
auto intHolder = ObjectPtr<GenericHolder>(new GenericHolder(dart_int(42)));
