...
auto multiline = dart_string("这是多行字符串\n可以包含多行内容\n");
...
auto describePerson = [&](String name, Int age, String hobby) -> String auto description = dart_string("姓名: ") + (name).toString();
if (!(dart_is_null(age))) {
description = description->operator_add(dart_string(", 年龄: ") + (age).toString());
}
if (!(dart_is_null(hobby))) {
description = description->operator_add(dart_string(", 爱好: ") + (hobby).toString());
}
return description;;
auto createAddress = [&]() -> String auto address = dart_string("城市: ") + (city).toString();
if (!(dart_is_null(street))) {
address = address->operator_add(dart_string(", 街道: ") + (street).toString());
}
if (!(dart_is_null(zipCode))) {
address = address->operator_add(dart_string(", 邮编: ") + (zipCode).toString());
}
return address;;
...
dart_print(describePerson(dart_string("赵六"), dart_int(25), dart_string("编程")));
dart_print(createAddress(/*city:*/ dart_string("北京"), /*street:*/ dart_string("长安街")));
return Void;
}
