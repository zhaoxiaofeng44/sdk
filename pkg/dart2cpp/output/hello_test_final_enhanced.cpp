...
Nullable testFinalFunctions() {
  dart_print(dart_string("\n--- 最终函数特性 ---"));
auto greetName = [&](String name) -> String return dart_concat(dart_string("你好, "), (name).toString(), dart_string("!"));;
auto addNumbers = [&](Int a, Int b) -> Int return a->operator_add(b);;
...
Nullable testFinalGenerics() {
  dart_print(dart_string("\n--- 最终泛型特性 ---"));
auto stringContainer = ObjectPtr<FinalContainer>(new FinalContainer(dart_string("Hello")));
auto intContainer = ObjectPtr<FinalContainer>(new FinalContainer(dart_int(42)));
