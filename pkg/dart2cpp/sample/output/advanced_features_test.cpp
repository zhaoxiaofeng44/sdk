...
dart_print(dart_concat(dart_string("    捕获的值: "), (functions->operator_index(dart_int(0))->apply(std::vector<Any>{})).toString(), dart_string(", "), (functions->operator_index(dart_int(1))->apply(std::vector<Any>{})).toString(), dart_string(", "), (functions->operator_index(dart_int(2))->apply(std::vector<Any>{})).toString()));
...
R LetExtension|let(T #this, std::function<R(T)> block) {
  return block->apply(std::vector<Any>{this});
}

std::function<Any(std::function<Any(T)>)> LetExtension|get#let(T #this) {
  return makeFunction([&](std::function<R(T)> block) { return LetExtension|let(this, block); });
}
