...
Any value4 = Map<String, String>::create({{dart_string("key"), dart_string("value")}});
...
if ((dart_is<String>(testValue)).operator_not()) {
dart_print(dart_string("    42 不是 String"));
}
if ((dart_is_null(testValue)).operator_not()) {
dart_print(dart_string("    42 不是 null"));
}
dart_print(dart_string("  复杂类型检查:"));
auto mixedList = dart_literal<Int>(dart_int(1), dart_string("hello"), dart_literal<Int>(dart_int(1), dart_int(2)), Map<String, String>::create({{dart_string("key"), dart_string("value")}}));
auto sync_for_iterator_1 = mixedList->iterator();
for (; sync_for_iterator_1->hasNext(); ) {
Any item = sync_for_iterator_1->next();
if (dart_is<Int>(item)) {
...
try {
auto parsedInt = Int::parse(intString);
...
// Finally block should be implemented using RAII pattern
try {
auto invalidInt = Int::parse(invalidString);
...
try {
auto fromHex = Int::parse(dart_string("FF"), dart_int(16));
...
if ((dart_is_null(promotableString)).operator_not()) {
dart_print(dart_string("    提升后的字符串: ") + (promotableString->toUpperCase()).toString());
dart_print(dart_string("    提升后的长度: ") + (promotableString->get_length()).toString());
}
...
dynamicVar = Map<String, String>::create({{dart_string("key"), dart_string("value")}});
dart_print(dart_string("    改为Map: ") + (dynamicVar).toString());
dart_print(dart_string("  动态方法调用:"));
Any stringDynamic = dart_string("hello world");
dart_print(dart_string("    动态String方法: ") + (dart_cast<String>(stringDynamic)->toUpperCase()).toString());
Any listDynamic = dart_literal<Int>(dart_int(1), dart_int(2), dart_int(3), dart_int(4), dart_int(5));
dart_print(dart_string("    动态List方法: ") + (dart_cast<ObjectPtr<List<Any>>>(listDynamic)->get_length()).toString());
...
Any functionVar = makeFunction<Int, Int>(std::function<Int(Int)>([=](Int x) -> Int { return x->operator_mul(dart_int(2)); }));
...
if (dart_is<ObjectPtr<TypedFunction<Int, Int>>>(functionVar_promoted)) {
auto functionVar_promoted = dart_cast<ObjectPtr<TypedFunction<Int, Int>>>(functionVar);
...
if (dynamicList->every(makeFunction<Bool, Any>(std::function<Bool(Any)>([=](Any item) -> Bool { return dart_is<Int>(item); })))) {
...
auto objectList = dart_literal<Int>(dart_int(1), dart_string("hello"), dart_double(3.14), dart_bool(true));
auto sync_for_iterator_2 = objectList->iterator();
for (; sync_for_iterator_2->hasNext(); ) {
auto item = sync_for_iterator_2->next();
if (dart_is<Int>(item)) {
auto item_promoted = dart_cast<Int>(item);
dart_print(dart_string("    整数: ") + (item_promoted).toString());
} else {
...
dart_print(dart_string("    Dog List作为Animal List: ") + (animalList->get_length()).toString());
