...
public:
  ...
  String toString() {
    return dart_concat(dart_string("Box<"), (Type::of<T>()).toString(), dart_string(">("), (this->_value).toString(), dart_string(")"));
  }
...
auto numberStrings = mapList(numbers, makeFunction<String, Int>(std::function<String(Int)>([=](Int n) -> String { return dart_string("Number: ") + (n).toString(); })));
dart_print(dart_string("    转换结果: ") + (numberStrings->take(dart_int(3))->toList()).toString());
auto evenNumbers = filterList(numbers, makeFunction<Bool, Int>(std::function<Bool(Int)>([=](Int n) -> Bool { return (n->operator_mod(dart_int(2)) == dart_int(0)); })));
dart_print(dart_string("    偶数过滤: ") + (evenNumbers).toString());
auto sum = reduceList(numbers, makeFunction<Int, Int, Int>(std::function<Int(Int, Int)>([=](Int a, Int b) -> Int { return a->operator_add(b); })));
auto concatenated = reduceList(dart_literal<String>(dart_string("a"), dart_string("b"), dart_string("c")), makeFunction<String, String, String>(std::function<String(String, String)>([=](String a, String b) -> String { return a->operator_add(b); })));
...
auto animalHandler = makeFunction<Any, ObjectPtr<Animal>>(std::function<Any(ObjectPtr<Animal>)>([=](ObjectPtr<Animal> animal) -> Any { dart_print(dart_string("      处理动物: ") + (animal->name).toString()); }));
auto dogHandler = makeFunction<Any, ObjectPtr<Dog>>(std::function<Any(ObjectPtr<Dog>)>([=](ObjectPtr<Dog> dog) -> Any { dart_print(dart_concat(dart_string("      处理狗: "), (dog->name).toString(), dart_string(", 品种: "), (dog->breed).toString())); }));
...
auto sync_for_iterator_2 = animals->iterator();
for (; sync_for_iterator_2->hasNext(); ) {
auto animal = sync_for_iterator_2->next();
