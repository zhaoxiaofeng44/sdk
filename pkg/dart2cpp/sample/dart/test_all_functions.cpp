...
ObjectPtr<Function> operation = Null;

ObjectPtr<List<ObjectPtr<Function>>> operations = dart_literal<ObjectPtr<Function>>(makeFunction<Int, Int, Int>(std::function<Int(Int, Int)>(&add)), makeFunction<Int, Int, Int>(std::function<Int(Int, Int)>(&subtract)), multiply);
...
ObjectPtr<Function> getAddFunction() {
  return makeFunction<Int, Int, Int>(std::function<Int(Int, Int)>(&add));
}

ObjectPtr<Function> createMultiplier(Int _factor) {
  ObjectPtr<_ValueBox<Int>> factor(new _ValueBox<Int>(_factor));
  return makeFunction<Int, Int>(std::function<Int(Int)>([=](Int value) mutable -> Int { return value->operator_mul((*factor)); }));
}

Nullable testAnonymousFunction() {
  auto result = calculate(dart_int(10), dart_int(5), makeFunction<Int, Int, Int>(std::function<Int(Int, Int)>([=](Int a, Int b) -> Int { return a->operator_add(b); })));
dart_print(dart_string("匿名函数结果: ") + (result).toString());
return Void;
}

Nullable testFunctionAssignment() {
  operation = makeFunction<Int, Int, Int>(std::function<Int(Int, Int)>(&add));
