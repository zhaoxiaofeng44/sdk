#include "closure_value_boxing_test.h"
...
auto increment = makeFunction<Any>(std::function<Any()>([=]() -> Any { (*x) = (*x)->operator_add(dart_int(1)); }));
...
return makeFunction<Int>(std::function<Int()>([=]() mutable -> Int { (*start) = (*start)->operator_add(dart_int(1));
return (*start); })); };
...
auto incrementAndPrint = makeFunction<Any>(std::function<Any()>([=]() -> Any { (*count) = (*count)->operator_add(dart_int(1));
...
return makeFunction<Double, Int>(std::function<Double(Int)>([=](Int value) mutable -> Double { (*initial) = (*initial)->operator_add(value);
