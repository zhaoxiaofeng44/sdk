...
public:
  ...
  ObjectPtr<List<T>> sort(ObjectPtr<List<T>> items) {
    auto sorted = List::from(items, dart_bool(true));
sorted->sort();
return sorted;
  }
...
class GenericList {
private:
  ObjectPtr<List<T>> _items = _GrowableList::create(dart_int(0));
public:
  GenericList() {
  }
  ...
  ObjectPtr<List<T>> toList() {
    return List::from(this->_items, dart_bool(true));
  }
...
private:
  ObjectPtr<List<String>> _items = _GrowableList::create(dart_int(0));
public:
  StringRepository() {
  }
...
private:
  ObjectPtr<List<Int>> _items = _GrowableList::create(dart_int(0));
public:
  IntRepository() {
  }
...
auto strings = _GrowableList::_literal3(dart_string("a"), dart_string("b"), dart_string("c"));
...
auto concatenated = reduceList(_GrowableList::_literal3(dart_string("a"), dart_string("b"), dart_string("c")), makeFunction([&](String a, String b) { return a->operator_add(b); }));
...
dart_print(dart_string("    整数排序: ") + (intComparator->sort(_GrowableList::_literal5(dart_int(3), dart_int(1), dart_int(4), dart_int(1), dart_int(5)))).toString());
...
dart_print(dart_string("    列表大小: ") + (listProcessor->getSize(_GrowableList::_literal5(dart_int(1), dart_int(2), dart_int(3), dart_int(4), dart_int(5)))).toString());
...
Nullable testVariance() {
  dart_print(dart_string("\n📌 测试协变和逆变"));
auto animalList = _GrowableList::_literal2(/* Invalid: temp_dart_source.dart:259:32: Error: Too few positional arguments: 2 required, 1 given.
  ...
  var animalList = <Animal>[Dog('Buddy'), Cat('Whiskers')];
                                             ^ */);
auto dogList = _GrowableList::_literal2(/* Invalid: temp_dart_source.dart:260:26: Error: Too few positional arguments: 2 required, 1 given.
...
processAnimals(_GrowableList::_literal2(ObjectPtr<Dog>(new Dog(dart_string("Dog1"), dart_string("Beagle"))), ObjectPtr<Cat>(new Cat(dart_string("Cat1"), dart_string("Siamese")))));
