...
DART_AWAIT(Future::delayed(ObjectPtr<Duration>(new Duration(Int(Null), Int(Null), Int(Null), Int(Null), dart_int(50), Int(Null))), nullptr)->then(makeFunction([&](Any __wc0_formal) { return dart_print(dart_string("  then: 延迟操作完成")); }))->catchError(makeFunction([&](Any error) { return dart_print(dart_string("  catchError: ") + (error).toString()); })));
...
auto results = DART_AWAIT(Future::wait(futures, Bool(Null), nullptr));
...
auto numbers = DART_AWAIT(Future::wait(numberFutures, Bool(Null), nullptr));
...
auto chainResult = DART_AWAIT(Future::value(dart_int(5))->then(makeFunction([&](Int value) { return value->operator_mul(dart_int(2)); }))->then(makeFunction([&](Int value) { return dart_string("Result: ") + (value).toString(); }))->then(makeFunction([&](String text) { return text->toUpperCase(); })));
...
DART_AWAIT(Future::forEach(items, makeFunction([&](String item) { DART_AWAIT(Future::delayed(ObjectPtr<Duration>(new Duration(Int(Null), Int(Null), Int(Null), Int(Null), dart_int(50), Int(Null))), nullptr));
...
try {
auto result = DART_AWAIT(slowOperation()->timeout(ObjectPtr<Duration>(new Duration(Int(Null), Int(Null), Int(Null), Int(Null), dart_int(100), Int(Null)))));
...
auto controller = StreamController::(nullptr, nullptr, nullptr, nullptr, Bool(Null));
...
auto for_iterator = ObjectPtr<_StreamIterator>(new _StreamIterator(stream));
try { /* try block */ } catch (const std::exception& e) { /* catch block */ }
auto evenStream = Stream::fromIterable(List<Int>::createFromValues({dart_int(1), dart_int(2), dart_int(3), dart_int(4), dart_int(5), dart_int(6), dart_int(7), dart_int(8), dart_int(9), dart_int(10)}))->where(makeFunction([&](Int n) { return (n->operator_mod(dart_int(2)) == dart_int(0)); }));
dart_print(dart_string("  where过滤 (偶数):"));
auto stream = evenStream;
auto for_iterator = ObjectPtr<_StreamIterator>(new _StreamIterator(stream));
try { /* try block */ } catch (const std::exception& e) { /* catch block */ }
auto expandedStream = Stream::fromIterable(dart_literal(dart_int(1), dart_int(2), dart_int(3)))->expand(makeFunction([&](Int n) { return dart_literal(n, n->operator_mul(dart_int(10))); }));
...
Timer::periodic(ObjectPtr<Duration>(new Duration(Int(Null), Int(Null), Int(Null), Int(Null), dart_int(50), Int(Null))), makeFunction([&](ObjectPtr<Timer> timer) { count = count->operator_add(dart_int(1));
...
DART_AWAIT(Future::delayed(ObjectPtr<Duration>(new Duration(Int(Null), Int(Null), Int(Null), Int(Null), dart_int(10), Int(Null))), nullptr));
    DART_ASYNC_END
}
...
DART_ASYNC_FUNCTION(ObjectPtr<Future<String>>, fetchData, (String name, Int delayMs)) {
    DART_ASYNC_BEGIN
  DART_AWAIT(Future::delayed(ObjectPtr<Duration>(new Duration(Int(Null), Int(Null), Int(Null), Int(Null), delayMs, Int(Null))), nullptr));
...
DART_ASYNC_FUNCTION(ObjectPtr<Future<String>>, slowOperation, ()) {
    DART_ASYNC_BEGIN
  DART_AWAIT(Future::delayed(ObjectPtr<Duration>(new Duration(Int(Null), Int(Null), Int(Null), Int(Null), dart_int(200), Int(Null))), nullptr));
...
DART_AWAIT(Future::delayed(ObjectPtr<Duration>(new Duration(Int(Null), Int(Null), Int(Null), Int(Null), dart_int(100), Int(Null))), nullptr));
