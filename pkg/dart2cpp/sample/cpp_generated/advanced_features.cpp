...
class Point : virtual public Comparable<ObjectPtr<Point>> {
public:
  Double x;
  Double y;
...
return Void;
  ...
  Nullable validatePassword(String password) {
    dart_print(dart_string("    验证密码: ") + (password->replaceAll(dart_string("."), dart_string("*"))).toString());
return Void;
  }
...
namespace StringExtensions {
  inline String capitalize(String this_) {
    if (this_->isEmpty()) {
return this_;
}
return this_->operator_index(dart_int(0))->toUpperCase()->operator_add(this_->substring(dart_int(1)));
  }
  
  inline Bool isPalindrome(String this_) {
    auto cleaned = this_->toLowerCase()->replaceAll(dart_string("[^a-z0-9]"), dart_string(""));
return (cleaned == cleaned->split(dart_string(""))->reversed()->join(dart_string("")));
  ...
  inline String reverse(String this_) {
    return this_->split(dart_string(""))->reversed()->join(dart_string(""));
  }
...
namespace IntExtensions {
  inline Int factorial(Int this_) {
    if (this_->operator_less_equals(dart_int(1))) {
return dart_int(1);
}
return this_->operator_mul(IntExtensions::factorial(this_->operator_sub(dart_int(1))));
  }
  
  inline Bool isPrime(Int this_) {
    if (this_->operator_less_equals(dart_int(1))) {
return dart_bool(false);
}
if (this_->operator_less_equals(dart_int(3))) {
return dart_bool(true);
}
if ((this_->operator_mod(dart_int(2)) == dart_int(0)) || (this_->operator_mod(dart_int(3)) == dart_int(0))) {
return dart_bool(false);
}
for (auto i = dart_int(5); i->operator_mul(i)->operator_less_equals(this_); i = i->operator_add(dart_int(6))) {
if ((this_->operator_mod(i) == dart_int(0)) || (this_->operator_mod(i->operator_add(dart_int(2))) == dart_int(0))) {
return dart_bool(false);
}
}
return dart_bool(true);
  ...
  inline Int squared(Int this_) {
    return this_->operator_mul(this_);
  }
...
namespace ListExtensions {
  ...
  template<typename T>
  inline T random(ObjectPtr<List<T>> this_) {
    if (this_->isEmpty()) {
...
return (this_->get_year() == now->get_year()) && (this_->get_month() == now->get_month()) && (this_->get_day() == now->get_day());
  ...
  inline String formatDate(ObjectPtr<DateTime> this_) {
    return dart_concat((this_->get_year()).toString(), dart_string("-"), (this_->get_month()->toString()->padLeft(dart_int(2), dart_string("0"))).toString(), dart_string("-"), (this_->get_day()->toString()->padLeft(dart_int(2), dart_string("0"))).toString());
  }
  
  inline ObjectPtr<DateTime> addBusinessDays(ObjectPtr<DateTime> this_, Int days) {
    auto result = this_;
auto addedDays = dart_int(0);
while (addedDays->operator_less(days)) {
result = result->add(ObjectPtr<Duration>(new Duration(dart_int(1), dart_int(0), dart_int(0), dart_int(0), dart_int(0), dart_int(0))));
if (result->get_weekday()->operator_less(dart_int(6))) {
...
template<typename T>
ObjectPtr<TypedFunction<T, T>> pipe(ObjectPtr<List<ObjectPtr<Function>>> functions);
template<typename T, typename U, typename R>
...
String StringExtensions::reverse(String this_);
Int IntExtensions::factorial(Int this_);
Bool IntExtensions::isPrime(Int this_);
Int IntExtensions::squared(Int this_);
template<typename T>
T ListExtensions::secondOrNull(ObjectPtr<List<T>> this_);
template<typename T>
T ListExtensions::secondLastOrNull(ObjectPtr<List<T>> this_);
template<typename T>
T ListExtensions::random(ObjectPtr<List<T>> this_);
Bool DateTimeExtensions::isToday(ObjectPtr<DateTime> this_);
...
dart_print(dart_string("    是否为偶数: ") + (number->get_isEven()).toString());
...
auto m1 = ObjectPtr<Matrix>(new Matrix(dart_literal<ObjectPtr<List<Double>>>(dart_literal<Double>(dart_double(1.0), dart_double(2.0)), dart_literal<Double>(dart_double(3.0), dart_double(4.0)))));
auto m2 = ObjectPtr<Matrix>(new Matrix(dart_literal<ObjectPtr<List<Double>>>(dart_literal<Double>(dart_double(5.0), dart_double(6.0)), dart_literal<Double>(dart_double(7.0), dart_double(8.0)))));
...
template<typename T>
ObjectPtr<TypedFunction<T, T>> pipe(ObjectPtr<List<ObjectPtr<Function>>> functions) {
  return makeFunction<T, T>(std::function<T(T)>([=](T input) -> T { Any result = input;
auto sync_for_iterator_1 = functions->iterator();
for (; sync_for_iterator_1->hasNext(); ) {
auto func = sync_for_iterator_1->next();
result = dart_cast<ObjectPtr<Function>>(func)->call(result);
}
return dart_cast<T>(result); }));
}
...
Any result = dart_cast<ObjectPtr<Function>>(func)->call(arg);
...
// yield i;  // C++20 coroutine not supported in C++17
}
}
