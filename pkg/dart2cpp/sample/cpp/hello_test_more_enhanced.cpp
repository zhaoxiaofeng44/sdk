...
auto strLength = name->get_length();
...
Nullable testMoreFunctions() {
  dart_print(dart_string("\n--- 更多函数特性 ---"));
auto greeter = makeFunction<String, String>(std::function<String(String)>([=](String name) -> String { return dart_concat(dart_string("你好, "), (name).toString(), dart_string("!")); }));
...
auto multiply = [&](Int a, Int b) { return a->operator_mul(b); };
...
Nullable testMoreCollections() {
  dart_print(dart_string("\n--- 更多集合特性 ---"));
auto numbers = dart_literal<Int>(dart_int(1), dart_int(2), dart_int(3), dart_int(4), dart_int(5));
dart_print(dart_string("列表长度: ") + (numbers->get_length()).toString());
...
dart_print(dart_string("添加元素后长度: ") + (numbers->get_length()).toString());
auto scores = Map<String, Int>::create({{dart_string("张三"), dart_int(95)}, {dart_string("李四"), dart_int(87)}, {dart_string("王五"), dart_int(92)}});
dart_print(dart_string("映射大小: ") + (scores->get_length()).toString());
...
scores->addAll(Map<String, Int>::create({{dart_string("孙七"), dart_int(90)}, {dart_string("周八"), dart_int(85)}}));
dart_print(dart_string("更新后映射大小: ") + (scores->get_length()).toString());
auto uniqueNames = ([&]() { const auto unnamed_var = ObjectPtr<Set<String>>(new Set<String>()); unnamed_var->add(dart_string("张三")); unnamed_var->add(dart_string("李四")); unnamed_var->add(dart_string("王五")); return unnamed_var; })();
dart_print(dart_string("集合大小: ") + (uniqueNames->get_length()).toString());
uniqueNames->add(dart_string("赵六"));
dart_print(dart_string("添加后集合大小: ") + (uniqueNames->get_length()).toString());
return Void;
}
...
Nullable testMoreGenerics() {
  dart_print(dart_string("\n--- 更多泛型特性 ---"));
auto stringContainer = ObjectPtr<GenericContainer<String>>(new GenericContainer<String>(dart_string("Hello")));
auto intContainer = ObjectPtr<GenericContainer<Int>>(new GenericContainer<Int>(dart_int(42)));
...
if ((dart_is_null(nullableString)).operator_not()) {
...
dart_print(dart_string("枚举值: ") + (ObjectPtr<Color>(new Color())).toString());
