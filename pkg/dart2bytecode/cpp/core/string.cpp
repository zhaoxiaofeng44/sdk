...
StringPoolEntry::StringPoolEntry()
    : data(NULL), length(0), refCount(0), inUse(false) {}
...
bool StringPool::stringEqual(const uint8_t* s1,
                             const uint8_t* s2,
                             int len1,
                             int len2) {
...
StringPoolEntry* StringPool::intern(const char* str) {
  ...
  for (int i = 0; i < poolSize && i < MAX_POOL_SIZE; i++) {
    if (pool[i].inUse &&
        stringEqual(pool[i].data, reinterpret_cast<const uint8_t*>(str),
...
StringPoolEntry* StringPool::intern(const uint8_t* str) {
  ...
  for (int i = 0; i < poolSize && i < MAX_POOL_SIZE; i++) {
    if (pool[i].inUse &&
        stringEqual(pool[i].data, str, pool[i].length, length)) {
      pool[i].refCount++;
      return &pool[i];
    }
  }
...
Bool* String::cpp_equals(String* a, String* b) {
  ...
  return new Bool(StringPool::stringEqual(
      ...
      reinterpret_cast<const uint8_t*>(b->c_str()), a->length(), b->length()));
}
...
String* String::cpp_subscript(String* str, Int* index) {
  if (!str || !index || index->getInt() < 0 ||
      index->getInt() >= str->length()) {
    return new String("");
  }
...
String* String::replaceRange(String* str,
                             Int* start,
                             Int* end,
                             String* replacement) {
...
String* String::fromCharCodes(Int** charCodes, Int* length) {
  ...
  for (int i = 0; i < len; i++) {
    buffer[i] =
        charCodes[i] ? static_cast<uint8_t>(charCodes[i]->getInt()) : '\0';
  }
  buffer[len] = '\0';
...
String* String::extract(String* str, String* pattern) {
  ...
  String* result =
      match->m_data ? new String(pattern->c_str()) : new String("");
  delete match;
  return result;
}
